...
class ProjectParserTestCase(ParserTestCase):
    ...
    def get_last_project(self):
        ...
        projects = parser.get_projects(companies_page)

        self.assertEquals(len(projects), 42)

        project = projects[-1]
        self.assertEquals(project['name'], "Zilics")
        self.assertEquals(project['original_id'], 26)
        self.assertEquals(project['description'], "")
